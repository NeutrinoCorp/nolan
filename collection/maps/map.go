...
// Map An structure that maps keys to values. A map cannot contain duplicate keys; each key can map to at most one value.
type Map[K comparable, V any] interface {
	...
	Clear()
	// Keys Returns a collection.Collection view of the keys contained in this map.
	Keys() collection.Collection[K]
	// Values Returns a collection.Collection view of the values contained in this map.
	Values() collection.Collection[V]
	// ForEach iterates through all mappings from this map. Use predicate's return boolean value to indicate
